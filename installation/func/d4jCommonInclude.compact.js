...
function getBackgroundColor(root){var bgColor='';if(root!=null){if(typeof root.style!=''){if(typeof root.style.backgroundColor!='undefined'&&root.style.backgroundColor!=''){bgColor=root.style.backgroundColor;}}else if(typeof root.bgColor!='undefined'&&root.bgColor!=''){bgColor=root.bgColor;}else{bgColor=getBackgroundColor(root.parentNode);}}
return bgColor;}
function hideFrame(id){document.getElementById(id).style.display='none';}
function showFrame(id){document.getElementById(id).style.display='block';}
function getElementHeight(docObj,Elem){if(docObj.getElementById){var elem = docObj.getElementById(Elem);}else if(docObj.all){var elem = docObj.all[Elem];}if(elem!=null)return elem.offsetHeight;else return 0;}	
function getElementWidth(docObj,Elem){if(docObj.getElementById){var elem = docObj.getElementById(Elem);}else if(docObj.all){var elem = docObj.all[Elem];}if(elem!=null)return elem.offsetWidth;else return 0;}	 
var d4j_common_include_included=1;}
function str_replace(longStr,searchStr,replaceStr){var re = new RegExp(searchStr,"g");return longStr.replace(re,replaceStr);}