...
function mosShowListMenu($menutype) {
  ...
  include_once( _TEMPLATE_PATH.'/js/ie.js' );

	$indents = array(
	// block prefix / item prefix / item suffix / block suffix
	array( "<ul id=\"nav\">", "<li>" , "</li>", "</ul>" ),
	array( "<ul>", "<li>" , "</li>", "</ul>")
	);
...
function mosRecurseListMenu( $id, $level, &$children, $open, &$indents, $class_sfx, $highlight ) {
	...
	if (@$children[$id]) {
		...
		for ($_i = 0, $_n = count($children[$id]); $_i < $_n; $_i++) {
				$row =& $children[$id][$_i];
...
/* Hack by Nguyen Manh Cuong at DesignForJoomla.com */
if ($_i == ($_n - 1) AND $level > 0) {
	echo '<li style="width:629px">&nbsp;</li>';
}
/* End hack */
        }
		echo "\n".$indents[$n][3];
