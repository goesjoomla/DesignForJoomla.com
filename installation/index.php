...
<?php } else { ?>
          #rbox{width:1004px}
          #top,#bottom{margin:0 0 0 28px;width:945px}
          #mainbody{margin:0 0 10px 28px;width:915px}
          #lbox{width:0px}
<?php } ?>
<?php if (mosCountModules ('user9')) { ?>
          #footer_container{height:70px}
          #footer{width:760px}
          #bottom_right{margin:572px 0 0 0}
<?php } else { ?>
          #footer_container{height:70px}
          #footer{width:950px}
          #user9{margin:0}
...
<?php } else { ?>
          #rbox{width:1004px;}
          #top,#bottom{margin:0 0 0 14px;width:945px}
          #mainbody{margin:0 0 10px 14px;width:945px}
          #lbox{width:0px}
<?php } ?>
</style>
<![endif]-->
...
<div id="content">
    <div id="header">
       <div id="title"><?php if (mosCountModules('user7')) mosLoadModules('user7', -2);
           else echo '<h1>Lazy Days</h1>
                      <h2>in the city</h2>' ?>
       </div>
    </div>

    <div id="mainMenu"><?php $database->setQuery("SELECT id,name,link,type,browserNav FROM #__menu WHERE menutype = 'mainmenu' AND published = 1 AND access <= $my->gid AND parent = 0 ORDER BY ordering LIMIT 0,5");
