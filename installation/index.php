...
$topModules = (mosCountModules('inset') ? 1 : 0) + (mosCountModules('user5') ? 1 : 0) + (mosCountModules('user6') ? 1 : 0) + (mosCountModules('user7') ? 1 : 0);
...
<?php } ?>
<?php if (mosCountModules('left') OR mosCountModules('right') OR mosCountModules('user4') OR mosCountModules('user3') OR mosCountModules('newsflash')) { ?>
        #lbox{width:510px}
        #rbox{width:380px}
<?php } else { ?>
        #rbox{width:0%;height:0}
        #lbox,#top,#box1,#user1,#user2,#banner,#mainbody,#bottom{width:910px}
        #content{background:none}
<?php } ?>
<?php if (mosCountModules ('user9')) { ?>
        #footer{width:500px;height:45px}
        #user9{width:390px}
        #bottom_right{width:0%;height:0%;margin:100px 0 0;padding:0}
<?php } else { ?>
        #footer{width:890px}
        #user9{width:0px;height:0px}
        #bottom_right{width:auto;height:auto;margin:0}
<?php } ?>
<?php if ($topModules > 0) { ?>
<?php } if ($topModules == 4) { ?>
        #user5,#user6,#user7,#inset{width:24.97%}
<?php } elseif ($topModules == 3) { ?>
        #user5,#user6,#user7,#inset{width:33%}
<?php } elseif ($topModules == 2) { ?>
        #user5,#user6,#user7,#inset{width:49.97%}
<?php } elseif ($topModules == 1) { ?>
        #user5,#user6,#user7,#inset{width:99.97%}
...
<div id="container">
        <div id="topbox">
                <div id="header"><?php if (mosCountModules('header')) mosLoadModules('header', -2);
                        else echo '<h1>newave</h1><h2>by free css templates</h2>' ?>
                </div>
        ...
        <?php if ($topModules > 0) { ?><div id="box3">
                <?php if (mosCountModules('inset')) { ?><div id="inset"><?php mosLoadModules('inset', -2);?></div><?php } ?>
