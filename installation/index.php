...
#left{display:none}
#center{width:720px;margin-right:8px}
<?php }?>
</style>
...
<div id="container">		
			...
			$database->setQuery("SELECT id,name,link,type,browserNav FROM #__menu WHERE menutype = 'mainmenu' AND published = 1 AND access <= $my->gid AND parent = 0 ORDER BY ordering LIMIT 0,6");
				if ($rows = $database->loadObjectList()) {
					for ($i = 0, $n = count($rows); $i < $n; $i++) {
						...
						if($i==$n-1) echo '<a href="'.$link.'" class="last">'.$rows[$i]->name.'</a>'; else echo '<a href="'.$link.'">'.$rows[$i]->name.'</a>';
				}				
				} 			
			?>
	</div>
	<div id="header">	
		...
		<div id="slogan">
			<?php if (mosCountModules('user7')) mosLoadModules('user7', -2);
					else echo '<h1 title="'.$GLOBALS['mosConfig_sitename'].'"><a href="'.$GLOBALS['mosConfig_live_site'].'" title="'.$GLOBALS['mosConfig_sitename'].'">'.$GLOBALS['mosConfig_sitename'].'</a></h1><h2>A short slogan come here</h2>'?>
		</div>		
	</div>
	...
	<div id="body">
		...
		<div id="footer"><?php if (mosCountModules('footer')) mosLoadModules('footer', -1); else include_once(_TEMPLATE_PATH.'/css/bottom.css.php'); ?>
		</div>	
	</div>
</center>
